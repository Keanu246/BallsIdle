...
function levelBall(id){ 
  ...
  if(value>=Math.floor(tempBall.cost*Math.pow(baseCostMulti,tempBall.rank))){ 
    ...
    if(notationScientfic){ 
        document.getElementById(id+"C").innerHTML = numberformat.format(Math.floor(tempBall.cost*Math.pow(1.1,tempBall.rank)),{format: 'scientific'}); 
    } 
    else{ 
        document.getElementById(id+"C").innerHTML = numberformat.format(Math.floor(tempBall.cost*Math.pow(1.1,tempBall.rank))); 
    } 
